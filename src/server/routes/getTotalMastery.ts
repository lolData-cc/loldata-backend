...
export async function getTotalMasteryHandler(req: Request): Promise<Response> {
  try {
    ...
    const masteryList: MasteryEntry[] = await masteryRes.json();

    // Total mastery points (somma championPoints)
    const totalPoints = masteryList.reduce((sum, m) => sum + (m.championPoints ?? 0), 0);

    // opzionale: top 5 champ per punti
    const top = [...masteryList]
    ...
    return Response.json({
      ...
      champions_count: masteryList.length,
      top,
    });
  } catch (err) {
    console.error("Errore in getTotalMasteryHandler:", err);
