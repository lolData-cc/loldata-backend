...
export async function aiChatHandler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const sys = ctx.matchId
      ? `${SYSTEM}\n\nATTACHED GAME: the user has ONE specific game pinned to this conversation (internal id ${ctx.matchId}). This is the CURRENT game and it MAY be DIFFERENT from any game discussed earlier in the chat. For any question about this game, the user's performance, laning, build/itemization, deaths, objectives or teamfighting, you MUST call my_game now and base that answer ONLY on the champion, comps, timeline and numbers it returns for THIS game — never reuse, continue or copy the analysis of a previously-attached game, even if the question is word-for-word identical to an earlier one. If the champion or stats differ from earlier in the chat, the attached game is the new and ONLY one that matters. Only if the user clearly asks something unrelated to this specific game, answer that normally.`
      : SYSTEM;
