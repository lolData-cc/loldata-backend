  ...
  serve({
    ...
    async fetch(req) {
      ...
      if (pathname === "/api/leaderboard" && req.method === "POST") return withCors(await getLeaderboardHandler(req));
      if (pathname === "/api/billing/create-checkout-session" && req.method === "POST") return withCors(await createCheckoutSessionHandler(req));
      if (pathname === "/api/webhooks/stripe" && req.method === "POST") return await stripeWebhookHandler(req);
      // === FILE STATICI ===
      try {
